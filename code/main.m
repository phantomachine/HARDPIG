...
% % DEPENDENCIES:
% %
% %     * KSMOD2 class for model and SSE solution.
...
iter_last = 438;                % Check number in /_mat folder
...
elseif strcmp(LOAD_OLD,'yes') == 1
    ...
    load(strcat('_mat/sse_iter',int2str(iter_last),'.mat'));
...
while epsi >= model.TOL1 && iter <= ( iter_last + model.MAXITER1 )
    ...
    C_new = admit(model, C_old, D, K, L, H, BxA, row_BxA, ...
    ...
    if epsi < 1
        filename = strcat('_mat/sse_iter',num2str(iter),'.mat');
        save(filename)
    end
