% % Class -- KSMOD02.M
% % =======================================================================
% %     (c) 2009-- T.Kam and R.Stauber.
% %         Defines class for model and SPE solution.
% %         See Kam and Stauber (2009).
% %         Email: timothy.kam@anu.edu.au || ronald.stauber@anu.edu.au
% % =======================================================================
% % $Revision: 4.0.3 $  $Date: 2011/03/08 11:38:20 $ 

classdef ksmod02 < handle
    ...
    properties
        ...
        AGRID;      % Discretized space for A_j^NS (action profile space)
        BGRID;      % Grid array for all b's (govt action space)
        
        NGRID_P;    % #elements in Grid approximating [0,1]
        NA = 3;     % No. of grid points for AGRID 
        NB = 15;     % No. grid points for each b(i), i = -N,...,-1,1,...,M 
    ...
    methods
        function self = ksmod02(SIGMA,PHI,M,N,WAGE,REP_RATIO,DELTA,NGRID_P)
            % KSMOD02: 
            ...
            % Agent Action set
            a_j = linspace(0.01, amax(self), self.NA)'; % j's action set
            self.A_j = a_j;
            ...
            b_u = linspace(0,self.MBAR,self.NB)';
            b_e = linspace(-self.WAGE*0.99,self.MBAR,self.NB)';
            ...
            self.BGRID = gridmake(bsu,bse); % requires GRIDMAKE: COMPECON
        end
