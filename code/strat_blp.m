...
for t = 1 : T
    ...
    for b_idx = 1 : NB             % Loop of feasible policies b in B_temp
        ...
        parfor a_idx = 1 : NA       % loop over agents' actions a in A
            ...
            g = sdpvar(1,NZ_local-1);    % (1 x N) search subgradient
            
            % Define YALMIP objective function -- bilinear in (l,w)
            E_abw = ( Util_now' + delta_local*P*w ); % vector:agent payoffs
            objective = g*E_abw; % bilinear form
            
            % YALMIP constraints
            
            v_temp = lambda_now*E_abw;
            
            constraint = [ G_next*w <= c_next,...     % (1) L constraints
                           v_now_local == v_temp,...  % (2) Bellman
                           -l * b' == 0, ...   % (3) 1 constraint
                           DevP*w <= DevA,...  % (4) NZ*(card(A)-1) constrs
                           sum(g*g') == 1,...   % (5) g; normalized gradient
            ...
            if sol.problem == 0 || sol.problem == 3
                ...
                %yalmiperror(sol.problem)
                val_a(a_idx) = -inf;
                wnext_a{a_idx} = -inf*ones(NZ_local,1);
            end
