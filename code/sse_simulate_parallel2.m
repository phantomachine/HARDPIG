...
if strcmp(load_old, 'on')
    ...
    M_theta = 688;                              % Inner ray subgradients
    ...
    %Theta = H;
    %M_theta = size(Theta,2);
    %Theta = [Theta; setdiff(Theta,H,'rows')];
    %M_theta = size(Theta,2);
    ...
    for t = 1:T
        ...
        if l_star > 0
            ...
            % Rays for Set of vertices of W:
            X = VertSetChull{k_now};
            q = (X - repmat(wko', size(X,1), 1));
            norm_q = repmat(sqrt(sum(q.^2, 2)), 1, size(X,2));
            RaysVerts = q ./ norm_q;
            Theta = [ Theta; RaysVerts ];
            ...
            % (3) Now given stored extreme points, find probability weights
                ...
                Constraints_lhs = [ Z_ext;             % 'S' == equalities
        ...
        fprintf('\n t = %i | k = %i | l* = %6.4g',t, k_now, l_star);
        fprintf('\n\ta_star \t b_star \t lam_next \t w_next\n');
        disp([a_star', b_star', lambda_next', w_next]);
