...
%% STARTUP RUN OPTIONS
LOAD_OLD = 'no';               % load W_old1.mat
iter_last = 2;                  % Check number in /_mat folder
...
%% PROGRAM SETTINGS
    ...
    plot_partsiminverse = 'on';  % Actual rv's in each Q(j)
...
elseif strcmp(LOAD_OLD,'yes') == 1
    ...
    load(strcat('_mat/sse2014_iter',int2str(iter_last),'.mat'));
...
%                                 epsi, iter );
 ...
 %   tic                    
    C_new = admit_outer_lpset_ramsey(model, xTriIndex, F, H,C_old, P_a, ...
                                              lb, ub, vartype, ...
                                              epsi, iter);
%    toc
