...
iter_last = 46;                  % Check number in /_mat folder
...
elseif strcmp(LOAD_OLD,'yes') == 1
    ...
    % Initial guess of hyperplane levels (payoff correspondence W_outer)
    load(strcat(pwd,'/_mat/ramsey2014_iter',int2str(iter_last),'.mat'));
...
%                                 epsi, iter );
 ...
 %   tic                    
    [Cnew, lambda_new, w_new ] = admit_outer_lpset_ramsey(model, xTriIndex, F, H, C_old, P_a,...
                                              lb, ub, vartype, ...
                                              epsi, iter);                                                                                
%    toc
    ...
    %if epsi < 1
        filename = strcat('_mat/ramsey2014_iter',num2str(iter),'.mat');
        save(filename)
    %end
