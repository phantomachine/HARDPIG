...
iter_last = 160;                  % Check number in /_mat folder
...
if strcmp(LOAD_OLD,'no') == 1
    ...
    % NOTES: Dependant parameters set in KSMOD3.M
    % -------------------------------------------
    ...
    fprintf('\nCreating MODEL instance from KSMOD03 ... Please wait.\n');

    model = ksmod03(SIGMA,PHI,M,N,WAGE,REP_RATIO,DELTA); 
...
elseif strcmp(LOAD_OLD,'yes') == 1
    ...
    fprintf('MAIN.M: Starting with old files! ... C_new_%i\n',iter_last);

    % Initial guess of hyperplane levels (payoff correspondence W_outer)
    load(strcat(pwd,'/_mat/sse2014_iter',int2str(iter_last),'.mat'));
    epsi = eps_vec(iter_last);
    iter = iter_last + 1;
...
iter_last_stop = iter_last;
clear iter_last

while epsi >= model.TOL1 
    
    if iter > ( iter_last_stop + model.MAXITER1 )
        break
    end
...
%                                 epsi, iter );
 ...
 %   tic                    
    [Cnew, lambda_new, w_new ] = admit_outer_lpset(model, xTriIndex, ...
                                              F, H, C_old, P_a, pival,...
                                              lb, ub, vartype, ...
                                              epsi, iter);
%    toc
    ...
    %if epsi < 1
        filename = strcat('_mat/sse2014_iter',num2str(iter),'.mat');
        save(filename)
    %end
...
AverageTimePerOperator = TotalElapsedTime/(iter - 1 - iter_last_stop);
save _mat/sse2014_final

fprintf('\nTotal Time = %5.6g sec\n', TotalElapsedTime);
fprintf('Average per SSE-operator iter = %5.6g sec\n', ...
                                        AverageTimePerOperator);
